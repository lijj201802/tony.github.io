var Message = function(msg) {
	...
	this.draw = function(context,x,y,i) {
		...
		var messageBox = {
			...
			x: x + 10,
			y: (y - i * (fontsize + paddingH * 2 +1))-10
		}
		...
		context.fillStyle = 'rgba(255,255,0,'+opacity/20+')';
		drawRoundedRectangle(context, messageBox.x, messageBox.y, messageBox.width, messageBox.height, 10);
		// context.fillStyle = 'rgba(255,255,255,'+opacity+')';
		 context.fillStyle = 'rgba(255,0,0,'+opacity+')';//tony
		context.fillText(message.message, messageBox.x + paddingW, messageBox.y + paddingH + 2, 100);
	}
